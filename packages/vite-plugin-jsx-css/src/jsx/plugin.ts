    ...
    JSXElement(path) {
      ...
      if (
        ...
        opening.attributes.push(
          t.jsxAttribute(t.jsxNamespacedName(t.jsxIdentifier(JSX_CSS_NS), t.jsxIdentifier(value)))
        );
      }
    },
  });
...
export function jsxCssPlugin(): babel.PluginObj<babel.PluginPass & { opts: { css: string } }> {
      ...
      Program(rootPath, state) {
        ...
        rootPath.traverse({
          ReturnStatement(path) {
            if (
              t.isTaggedTemplateExpression(path.node.argument) &&
              ...
              if (containingFunc) {
                ...
                if (!hasStyledJsx) {
                  ...
                  const importDecl = t.importDeclaration(
                    [],
                    t.stringLiteral(`virtual:${PLUGIN_NAME}/${hashedFileName}.css`)
                  );
